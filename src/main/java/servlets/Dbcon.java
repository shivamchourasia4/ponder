...
//			}
	...
	public ResultSet Searchtopic(String srctopic) throws SQLException {
		...
		String getSearchResults = "select posts_storage.dt, posts_storage.username, posts_storage.topic, posts.post from posts_storage inner join posts on posts_storage.posts_id=posts.post_id where topic like '%"
				+ srctopic + "%' order by posts_storage.dt desc";
		Statement statement = connection.createStatement();
	...
	public ResultSet ShowMyposts(String itspost) throws SQLException {
		...
		String getSearchResults = "select posts_storage.dt, posts_storage.username, posts_storage.topic, posts.post from posts_storage inner join posts on posts_storage.posts_id=posts.post_id where username='"
				+ itspost + "' order by posts_storage.dt desc";
		Statement statement = connection.createStatement();
		ResultSet resultSet = statement.executeQuery(getSearchResults);
		return resultSet;
	}
